...
#' \item{idrows}{List of row ids(cell ids) corresponding to responders and Non-responders} 
#' @seealso \code{\link{ccast_filter5}} which this function wraps
#' @export
#' @examples
#' ccast_heatmaplot2(optccastreeoutput=optccastree)
#'
ccast_heatmaplot2 <-
function(optccastreeoutput){
    ...
    ###par(mfrow=c(4,2)))
    homodata=list()
    tt2 <- party::nodes(tree3, c(1:max(unique(where(tree3))))[-unique(where(tree3))])
    
    splitstatname<- tt2[[1]]$psplit$variableName
    meanvarsplit=NULL
    for( j in 1:length(tt1)) {
        ...
        key_color1=seq(minv,maxv,length.out=dim(dD3[,-dim(dD3)[2],drop=FALSE])[2])
        
        rowids[[j]]=as.numeric(rownames(as.matrix(dD3)[w2,-dim(dD3)[2],drop=FALSE]))
        BB=as.matrix(rbind(dD3[w2,-dim(dD3)[2],drop=FALSE],key_color1))
        
        rownames(BB)=rep(paste("Cell",j),dim(BB)[1])
        mm <- round(c(apply(BB[-dim(BB)[1],,drop=FALSE], 2, mean)),3)
        MM <- rbind(MM,mm)
        mm2 <- round(c(apply(BB[-dim(BB)[1],,drop=FALSE], 2, sd)),3)
        MM2 <- rbind(MM2,mm2)
        ...
        homodata[[j]]=BB[-dim(BB)[1],,drop=FALSE]
        meanvarsplit=c(meanvarsplit,mean(homodata[[j]][,splitstatname]))
        rownames(homodata[[j]])=paste(rowids[[j]])
        rname=c(rname,paste("Node",tt1[[j]]$ nodeID))
        
    }
    sortmeanvar=sort(meanvarsplit,decreasing = FALSE)
    sortmeanvar2=match(meanvarsplit,sortmeanvar)
    print(paste("Apoptotic unsorted mean: ",meanvarsplit))
    print(paste("Increasing Apoptotic mean: ",sortmeanvar))
    homodata=homodata[sortmeanvar2]
    rowids=rowids[sortmeanvar2]
    rownames(MM)=rname
    rownames(MM2)=rname
    ...
    dev.off();
   return(list(ccastmeans=MM,ccastsds=MM2,idrows=rowids))
}
