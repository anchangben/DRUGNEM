...
fitnemupdownnoeffecttstat<-function(Out3,Outp,drugs,p=NULL,patient,infer,type="CONTmLL",targetprior){
    Deffect=list()
    Deffect2=list()
    Deffect3=list()
    Deffect4=list()
    ...
    for (t in 1:length(celltype)) {
        ...
        if(all(drugs%in% drugname)&(dim(fcp19)[2]>2*length(drugname))) {
            ...
            colnames(hdat2c)=drugname[-1]
            colnames(hdat2p)=drugname[-1]
            colnames(hdat2bp)=drugname[-1]
            colnames(hdat2cp)=drugname[-1]
            colnames(hdat3)=drugname[-1]
            colnames(hdat3p)=drugname[-1]
            hdat3_labelp=hdat3p
            ...
            Deffect_labelp[[t]]=hdat3_labelp[,drugs[-1]]
