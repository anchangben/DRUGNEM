...
fitnemupdownnoeffectFC<-function(Out3,Outp,drugs,p=NULL,patient,infer,type="CONTmLL",targetprior){
  ...
  # initialization
  list_names <- c("Deffect", "Deffect2", "Deffect3", "Deffect4", "Deffect4b", "Deffect5", "Deffect5b", "Deffect6", "Deffect6b", "Deffect7", 
                  "Deffect_label", "Deffect_labelp", "pop.a.c")
  cdata_names <- c("cdata1", "cdata2", "cdata2b", "cdata3", "cdata3b", "cdata6", "cdata6b", "cdata7")
  rr_names <- c("rr3", "rr4", "rr4b", "rr5", "rr5b", "rr6", "rr6b", "rr7")
  for (name in list_names) {assign(name, list())}
  for (name in cdata_names) {assign(name, NULL)}
  for (name in rr_names) {assign(name, NULL)}
  ...
  # initialize effect matrix: sub-pops x drugs
  Esize = matrix(0, nrow=length(celltype), ncol=(length(drugs)-1))
  colnames(Esize) = drugs[-1]
  rownames(Esize) = celltype
  
  # drug-nem for each sub-population
  for (t in 1:length(celltype)) {
    fcp19 = Out3[[t]]                    # protein x cell
    deathid = which(rownames(fcp19)==p)  # responder marker
    ...
    if(!(is.null(fcp19))) {
      if(all(drugs %in% drugname) & (dim(fcp19)[2]>2*length(drugname))) {
        ...
        Esize[t,]=Deffect3[[t]] # populate effect matrix with moderated t-stat
        
        # all other signaling markers
        dat2=fcp19[-deathid,]
        hdat=make.R.matrix1(dat2, wt=drugs[1], pi1 = 0.01)[[1]] # matrix of log odds
        hdat2=ifelse(Outp[-deathid,-1,t]>0,hdat,rnorm(1,-10,0.05)) # up-regulation
        hdat2b=ifelse(Outp[-deathid,-1,t]<0,hdat,rnorm(1,-10,0.05)) # down-regulation
        hdat2c=ifelse(Outp[-deathid,-1,t]==0,-hdat,rnorm(1,-10,0.05)) # no effect
        ...
        if(!(type %in% c("mLL", "CONTmLL"))) {
          ...
          if(targetprior==TRUE) {
            rr3=nem(Deffect2[[t]], inference = infer , 
                    control = set.default.parameters(unique(colnames(Deffect2[[t]])),type="CONTmLLBayes"), verbose=FALSE)
            rr4=nem(Deffect4[[t]], inference = infer , 
                    control = set.default.parameters(unique(colnames(Deffect4[[t]])),type="CONTmLLBayes"), verbose=FALSE)
            
            Deffect4b[[t]]=ifelse(Deffect_label[[t]]==1,Deffect7[[t]],rnorm(1,-10,0.05))
            rr4b=nem(Deffect4b[[t]], inference = infer , 
                     control = set.default.parameters(unique(colnames(Deffect4b[[t]])),type="CONTmLLBayes"), verbose=FALSE)
            rr5=nem(Deffect5[[t]], inference = infer , 
                    control = set.default.parameters(unique(colnames(Deffect5[[t]])),type="CONTmLLBayes"), verbose=FALSE)
            
            Deffect5b[[t]]=ifelse(Deffect_label[[t]]==2,Deffect7[[t]],rnorm(1,-10,0.05))
            rr5b=nem(Deffect5b[[t]], inference = infer , 
                     control = set.default.parameters(unique(colnames(Deffect5b[[t]])),type="CONTmLLBayes"), verbose=FALSE)
            rr6=nem(Deffect6[[t]], inference = infer , 
                    control = set.default.parameters(unique(colnames(Deffect6[[t]])),type="CONTmLLBayes"), verbose=FALSE)
            
            Deffect6b[[t]]=ifelse(Deffect_label[[t]]==3,Deffect7[[t]],rnorm(1,-10,0.05))
            rr6b=nem(Deffect6b[[t]], inference = infer , 
                     control = set.default.parameters(unique(colnames(Deffect6b[[t]])),type="CONTmLLBayes"), verbose=FALSE)
            rr7=nem(Deffect7[[t]], inference = infer , 
                    control = set.default.parameters(unique(colnames(Deffect7[[t]])),type="CONTmLLBayes"), verbose=FALSE)
          } else {
            rr3=nem(Deffect2[[t]], inference = infer , 
                    control = set.default.parameters(unique(colnames(Deffect2[[t]])),type="CONTmLLBayes",delta=0), verbose=FALSE)
            rr4=nem(Deffect4[[t]], inference = infer , 
                    control = set.default.parameters(unique(colnames(Deffect4[[t]])),type="CONTmLLBayes",delta=0), verbose=FALSE)
            
            Deffect4b[[t]]=ifelse(Deffect_label[[t]]==1,Deffect7[[t]],rnorm(1,-10,0.05))
            rr4b=nem(Deffect4b[[t]], inference = infer , 
                     control = set.default.parameters(unique(colnames(Deffect4b[[t]])),type="CONTmLLBayes",delta=0), verbose=FALSE)
            rr5=nem(Deffect5[[t]], inference = infer , 
                    control = set.default.parameters(unique(colnames(Deffect5[[t]])),type="CONTmLLBayes",delta=0), verbose=FALSE)
            
            Deffect5b[[t]]=ifelse(Deffect_label[[t]]==2,Deffect7[[t]],rnorm(1,-10,0.05))
            rr5b=nem(Deffect5b[[t]], inference = infer , 
                     control = set.default.parameters(unique(colnames(Deffect5b[[t]])),type="CONTmLLBayes",delta=0), verbose=FALSE)
            rr6=nem(Deffect6[[t]], inference = infer , 
                    control = set.default.parameters(unique(colnames(Deffect6[[t]])),type="CONTmLLBayes",delta=0), verbose=FALSE)
            
            Deffect6b[[t]]=ifelse(Deffect_label[[t]]==3,Deffect7[[t]],rnorm(1,-10,0.05))
            rr6b=nem(Deffect6b[[t]], inference = infer , 
                     control = set.default.parameters(unique(colnames(Deffect6b[[t]])),type="CONTmLLBayes",delta=0), verbose=FALSE)
            rr7=nem(Deffect7[[t]], inference = infer , 
                    control = set.default.parameters(unique(colnames(Deffect7[[t]])),type="CONTmLLBayes",delta=0), verbose=FALSE)
          }
        } else {
          # use estimated probabilities
          dat33=hdat2p[,drugs[-1]]
          dat33b=hdat2bp[,drugs[-1]]
          dat33c=hdat2cp[,drugs[-1]]
          dat33d=hdat3p[,drugs[-1]]
          # round to six decimal points
          dat31=round(dat33,6)
          dat31b=round(dat33b,6)
          dat31c=round(dat33c,6)
          dat31d=round(dat33d,6)
          # get number of entries in each df
          numdata=length(dat31)
          numdatab=length(dat31b)
          numdatac=length(dat31c)
          numdatad=length(dat31d)
          # generate `numdata` random absolute numbers 
          ss=abs(rnorm(numdata,0,0.005))
          ssb=abs(rnorm(numdatab,0,0.005))
          ssc=abs(rnorm(numdatac,0,0.005))
          ssd=abs(rnorm(numdatad,0,0.005))
          # replace NA values with (1 - randomly generated values)
          dat32=ifelse(is.na(dat31),1-ss,dat31)
          dat32b=ifelse(is.na(dat31b),1-ssb,dat31b)
          dat32c=ifelse(is.na(dat31c),1-ssc,dat31c)
          dat32d=ifelse(is.na(dat31d),1-ssd,dat31d)
          # additional processing
          dat34=ifelse(dat32>=0.99,dat32-ss,dat32)[,drugs[-1]]
          dat34b=ifelse(dat32b>=0.99,dat32b-ssb,dat32b)[,drugs[-1]]
          dat34c=ifelse(dat32c>=0.99,dat32c-ssc,dat32c)[,drugs[-1]]
          dat34d=ifelse(dat32d>=0.99,dat32d-ssd,dat32d)[,drugs[-1]]
          Deffect4[[t]]=dat34
          Deffect5[[t]]=dat34b
          Deffect6[[t]]=dat34c
          Deffect7[[t]]=dat34d
          ...
          if(targetprior==TRUE) {
            rr3=nem(Deffect2[[t]], inference = infer, 
                    control = set.default.parameters(unique(colnames(Deffect2[[t]])),type=type), verbose=FALSE)
            rr4=nem(Deffect4[[t]], inference = infer, 
                    control = set.default.parameters(unique(colnames(Deffect4[[t]])),type=type), verbose=FALSE)
            
            # keeping only max effects that originate from `hdat2p`, others set to ~0
            Deffect4b[[t]]=ifelse(Deffect_labelp[[t]]==1,Deffect7[[t]],abs(rnorm(1,0,0.005)))
            rr4b=nem(Deffect4b[[t]], inference = infer, 
                     control = set.default.parameters(unique(colnames(Deffect4b[[t]])),type=type), verbose=FALSE)
            
            rr5=nem(Deffect5[[t]], inference = infer, 
                    control = set.default.parameters(unique(colnames(Deffect5[[t]])),type=type), verbose=FALSE)
            
            # keeping only max effects that originate from `hdat2bp`, others set to ~0
            Deffect5b[[t]]=ifelse(Deffect_labelp[[t]]==2,Deffect7[[t]],abs(rnorm(1,0,0.005)))
            rr5b=nem(Deffect5b[[t]], inference = infer, 
                     control = set.default.parameters(unique(colnames(Deffect5b[[t]])),type=type), verbose=FALSE)
            
            rr6=nem(Deffect6[[t]], inference = infer, 
                    control = set.default.parameters(unique(colnames(Deffect6[[t]])),type=type), verbose=FALSE)
            
            # keeping only max effects that originate from `hdat2cp`, others set to ~0
            Deffect6b[[t]]=ifelse(Deffect_labelp[[t]]==3,Deffect7[[t]],abs(rnorm(1,0,0.005)))
            rr6b=nem(Deffect6b[[t]], inference = infer, 
                     control = set.default.parameters(unique(colnames(Deffect6b[[t]])),type=type), verbose=FALSE)
            
            rr7=nem(Deffect7[[t]], inference = infer, 
                    control = set.default.parameters(unique(colnames(Deffect7[[t]])),type=type), verbose=FALSE)
          } else {
            rr3=nem(Deffect2[[t]], inference = infer , 
                    control = set.default.parameters(unique(colnames(Deffect2[[t]])),type=type,delta=0), verbose=FALSE)
            rr4=nem(Deffect4[[t]], inference = infer , 
                    control = set.default.parameters(unique(colnames(Deffect4[[t]])),type=type,delta=0), verbose=FALSE)
            
            Deffect4b[[t]]=ifelse(Deffect_labelp[[t]]==1,Deffect7[[t]],abs(rnorm(1,0,0.005)))
            rr4b=nem(Deffect4b[[t]], inference = infer , 
                     control = set.default.parameters(unique(colnames(Deffect4b[[t]])),type=type,delta=0), verbose=FALSE)
            rr5=nem(Deffect5[[t]], inference = infer , 
                    control = set.default.parameters(unique(colnames(Deffect5[[t]])),type=type,delta=0), verbose=FALSE)
            
            Deffect5b[[t]]=ifelse(Deffect_labelp[[t]]==2,Deffect7[[t]],abs(rnorm(1,0,0.005)))
            rr5b=nem(Deffect5b[[t]], inference = infer , 
                     control = set.default.parameters(unique(colnames(Deffect5b[[t]])),type=type,delta=0), verbose=FALSE)
            rr6=nem(Deffect6[[t]], inference = infer , 
                    control = set.default.parameters(unique(colnames(Deffect6[[t]])),type=type,delta=0), verbose=FALSE)
            
            Deffect6b[[t]]=ifelse(Deffect_labelp[[t]]==3,Deffect7[[t]],abs(rnorm(1,0,0.005)))
            rr6b=nem(Deffect6b[[t]], inference = infer , 
                     control = set.default.parameters(unique(colnames(Deffect6b[[t]])),type=type,delta=0), verbose=FALSE)
            rr7=nem(Deffect7[[t]], inference = infer , 
                    control = set.default.parameters(unique(colnames(Deffect7[[t]])),type=type,delta=0), verbose=FALSE)
          }
        }
        
        # number of proteins in the graph with max log posterior marginal likelihood
        # since this always include those in "null" -> essentially all signaling proteins considered for the network
        if (infer == "search") {
        ...
        # probability effect matrix of proteins that were found in the optimal network
        # since this always include those in "null" -> essentially all signaling proteins considered for the network
        ...
        # assemble inputs for population NEM (cdata1, ..., cdata7)
        ...
        # plot nem
        TM_list = list(TM, TMs, TMs2, TMs3, TMs4, TMsb, TMs2b, TMs3b)
        Deffect_list = list(Deffect2[[t]], Deffect4[[t]], Deffect5[[t]], Deffect6[[t]], Deffect7[[t]], Deffect4b[[t]], Deffect5b[[t]], Deffect6b[[t]])
        rr_list = list(rr3, rr4, rr5, rr6, rr7, rr4b, rr5b, rr6b)
        title = c("", "Up", "Down", "No effect", "Best effect", "Up markers", "Down markers", "No effect markers")
        subpop = paste0("Pop", t)
        
        for (i in 1:length(TM_list)) {
          # plot option when the adjacency matrix is not empty 
          if ((sum(TM_list[[i]] != 0) > 1) & (edgemode(rr_list[[i]][["graph"]]) == "directed")) {
            plot.nem2(rr_list[[i]], 
                      filename=paste("subpop/",subpop,title[i]," DRUGNEM effect network and Heatmap for all states",infer,type,".pdf",sep=""),
                      main=paste0(title[i], " DRUGNEM effect Network"), PDF=TRUE, what = "graph",
                      D=Deffect_list[[i]], draw.lines = FALSE)
          } else {
            # otherwise invoke second plotting function
            pdf(file=paste("subpop/",subpop,title[i]," DRUGNEM effect network and Heatmap of pooled cells with",infer,type,patient, ".pdf", sep=""),
                width=15, height=15)
            plot(rr_list[[i]][["graph"]], main=paste0(title[i], " DRUGNEM effect Network"))
            par(mar=c(5,2,5,5))
            
            if (infer == "search") {
              nemselected = unique(unlist(rr_list[[i]][["mappos"]][[which.max(rr_list[[i]][["mLL"]])]]))
            } else {
              nemselected = unique(unlist(rr_list[[i]][["mappos"]]))
            }
            
            if(length(nemselected)!=0) {
              pic = Deffect_list[[i]][nemselected,,drop = FALSE]
              image(x=1:dim(pic)[2],y=1:dim(pic)[1],z=as.matrix(t(pic)),col = colorpalette, 
                    xlab = "Drugs", xaxt="n", yaxt="n", ylab = "target-proteins",family="sans",
                    cex.lab=1.4, main=paste("Heatmap","-","pooled"," drug effects", sep=""))
              mtext(rownames(pic),at=c(1:dim(pic)[1]),side=4,las=2,line=1, family= "sans", cex=0.5)
              mtext(colnames(pic),at=c(1:dim(pic)[2]),side=1,las=1,line=1, family= "sans", cex=1)
              dev.off();
            }
          }
        }
        ...
        # rank nem
        pdf(file=paste("subpop/",subpop," DRUG-COMBINATION scores.pdf", sep=""), width=13, height=13)
        for (i in 1:length(TM_list)) {
          out = bestdrugtarget(G=rr_list[[i]][["graph"]],
                               mappos=rr_list[[i]][["mappos"]][[which.max(rr_list[[i]][["mLL"]])]],
                               data=rr_list[[i]][["pos"]][[which.max(rr_list[[i]][["mLL"]])]])
          out_independence = scoredrugcombbliss(Deffect_list[[i]])
          
          Mat = matrix(out[[2]],nrow=length(out[[2]]))
          rownames(Mat) = names(out[[2]])
          textplot(round(Mat,4),cex.main=3,cex=2.5,family="sans")
          title(paste0(title[i], " DRUGNEM effect scores"))
          
          Mat1 = matrix(out_independence[[2]],nrow=length(out_independence[[2]]))
          rownames(Mat1)=names(out_independence[[2]])
          textplot(round(Mat1,4),cex.main=3,cex=2.5,family="sans")
          title(paste0(title[i], " Drug Independence effect Scores"))
        }
        dev.off()
      }
    }
  ...
  if(!(type %in% c("mLL", "CONTmLL"))) {
    ...
    if((sum(TM!=0)>1)&(edgemode(rr$graph)=="directed")) {
      ...
      pdf(file=paste(patient,"DRUGNEM effect network and Heatmap of pooled cells with",infer,type,patient, ".pdf", sep=""),width=15, height=20)
      plot(rr$graph,main="DRUGNEM effect network")
      par(mar=c(5,2,5,5))
      if (infer=="search") {
          nemselected= unique(unlist(rr$mappos[[which.max(rr$mLL)]]))
      ...
      if(length(nemselected)!=0) {
          ...
          mtext(rownames(pic2),at=c(1:dim(pic2)[1]),side=4,las=2,line=1, family= "sans", cex=0.3)
          mtext(colnames(pic2),at=c(1:dim(pic2)[2]),side=1,las=1,line=1, family= "sans", cex=1)
          dev.off();
      }
    }
    ...
    if((sum(TMs!=0)>1)&(edgemode(rr1$graph)=="directed")) {
        ...
        pdf(file=paste(patient,"Up DRUGNEM network and Heatmap of pooled cells  with",infer,type,patient, ".pdf", sep=""),width=15, height=20)
        ...
        if(length(nemselected)!=0) {
            ...
            mtext(rownames(pic3),at=c(1:dim(pic3)[1]),side=4,las=2,line=1, family= "sans", cex=0.3)
            mtext(colnames(pic3),at=c(1:dim(pic3)[2]),side=1,las=1,line=1, family= "sans", cex=1)
            dev.off();
        }
    ...
    if((sum(TMs2!=0)>1)&(edgemode(rr2$graph)=="directed")) {
        ...
        pdf(file=paste(patient,"Down DRUGNEM network and Heatmap of pooled cells  with",infer,type,patient, ".pdf", sep=""),width=15, height=20)
        ...
        if(length(nemselected)!=0) {
            ...
            image(x=1:dim(pic4)[2],y=1:dim(pic4)[1],z=as.matrix(t(pic4)),col = colorpalette,xlab = "Drugs",xaxt="n",yaxt="n",ylab = "target-proteins",family="sans",cex.lab=1.4,main=paste("Heatmap","-","pooled"," drug effects", sep=""))
            mtext(rownames(pic4),at=c(1:dim(pic4)[1]),side=4,las=2,line=1, family= "sans", cex=0.3)
            mtext(colnames(pic4),at=c(1:dim(pic4)[2]),side=1,las=1,line=1, family= "sans", cex=1)
            dev.off();
        }
    }
    ...
    if((sum(TMs3!=0)>1)&(edgemode(rr66$graph)=="directed")) {
        ...
        pdf(file=paste(patient,"No effect DRUGNEM network and Heatmap of pooled cells  with",infer,type,patient, ".pdf", sep=""),width=15, height=20)
        ...
        if(length(nemselected)!=0) {
            ...
            image(x=1:dim(pic6)[2],y=1:dim(pic6)[1],z=as.matrix(t(pic6)),col = colorpalette,xlab = "Drugs",xaxt="n",yaxt="n",ylab = "target-proteins",family="sans",cex.lab=1.4,main=paste("Heatmap","pooled"," drug No effects", sep=""))
            mtext(rownames(pic6),at=c(1:dim(pic6)[1]),side=4,las=2,line=1, family= "sans", cex=0.3)
            mtext(colnames(pic6),at=c(1:dim(pic6)[2]),side=1,las=1,line=1, family= "sans", cex=1)
            dev.off();
        }
    }
    ...
    if((sum(TMsb!=0)>1)&(edgemode(rr1b$graph)=="directed")) {
        ...
        pdf(file=paste(patient,"Up markers DRUGNEM network and Heatmap of pooled cells  with",infer,type,patient, ".pdf", sep=""),width=15, height=20)
        ...
        if(length(nemselected)!=0) {
            ...
            image(x=1:dim(pic3b)[2],y=1:dim(pic3b)[1],z=as.matrix(t(pic3b)),col = colorpalette,xlab = "Drugs",xaxt="n",yaxt="n",ylab = "target-proteins",family="sans",cex.lab=1.4,main=paste("Heatmap","-","pooled"," drug effects", sep=""))
            mtext(rownames(pic3b),at=c(1:dim(pic3b)[1]),side=4,las=2,line=1, family= "sans", cex=0.3)
            mtext(colnames(pic3b),at=c(1:dim(pic3b)[2]),side=1,las=1,line=1, family= "sans", cex=1)
            dev.off();
        }
    }
    ...
    #} else {
        pdf(file=paste(patient,"Down markers DRUGNEM network and Heatmap of pooled cells  with",infer,type,patient, ".pdf", sep=""),width=15, height=20)
        ...
        if(length(nemselected)!=0) {
            ...
            image(x=1:dim(pic4b)[2],y=1:dim(pic4b)[1],z=as.matrix(t(pic4b)),col = colorpalette,xlab = "Drugs",xaxt="n",yaxt="n",ylab = "target-proteins",family="sans",cex.lab=1.4,main=paste("Heatmap","-","pooled"," drug effects", sep=""))
            mtext(rownames(pic4b),at=c(1:dim(pic4b)[1]),side=4,las=2,line=1, family= "sans", cex=0.3)
            mtext(colnames(pic4b),at=c(1:dim(pic4b)[2]),side=1,las=1,line=1, family= "sans", cex=1)
            dev.off();
        }
    #}
    ...
    if((sum(TMs3b!=0)>1)&(edgemode(rr66b$graph)=="directed")) {
        ...
        pdf(file=paste(patient,"No effect markers DRUGNEM network and Heatmap of pooled cells  with",infer,type,patient, ".pdf", sep=""),width=15, height=20)
        ...
        if(length(nemselected)!=0) {
            ...
            image(x=1:dim(pic6b)[2],y=1:dim(pic6b)[1],z=as.matrix(t(pic6b)),col = colorpalette,xlab = "Drugs",xaxt="n",yaxt="n",ylab = "target-proteins",family="sans",cex.lab=1.4,main=paste("Heatmap","pooled"," drug No effects", sep=""))
            mtext(rownames(pic6b),at=c(1:dim(pic6b)[1]),side=4,las=2,line=1, family= "sans", cex=0.3)
            mtext(colnames(pic6b),at=c(1:dim(pic6b)[2]),side=1,las=1,line=1, family= "sans", cex=1)
            dev.off();
        }
    }
    ...
    if((sum(TMs4!=0)>1)&(edgemode(rr77$graph)=="directed")) {
        ...
        pdf(file=paste(patient,"Best effect DRUGNEM network and Heatmap of pooled cells  with",infer,type,patient, ".pdf", sep=""),width=15, height=20)
        ...
        if(length(nemselected)!=0) {
            ...
            image(x=1:dim(pic7)[2],y=1:dim(pic7)[1],z=as.matrix(t(pic7)),col = colorpalette,xlab = "Drugs",xaxt="n",yaxt="n",ylab = "target-proteins",family="sans",cex.lab=1.4,main=paste("Heatmap","pooled"," drug best effects", sep=""))
            mtext(rownames(pic7),at=c(1:dim(pic7)[1]),side=4,las=2,line=1, family= "sans", cex=0.3)
            mtext(colnames(pic7),at=c(1:dim(pic7)[2]),side=1,las=1,line=1, family= "sans", cex=1)
            dev.off();
        }
    }
