#' A function to predict the DRUGNEM network, scores drug combinations and plot diagnostic heatmaps for combination therapy using weighttype=="deathmarker" option.
#'
...
#' @export
#' @examples
#'res3=fitmnemupdownnoeffectdeathmarker(Out3=Sdata2,Outp=Out2,drugs=dids,p=NULL,patient=patient,infer=infer,type=type)
###### Fit,score and Plot DRUGNEM with heatmap on single pdf##########

fitnemupdownnoeffectdeathmarker<-function(Out3,Outp,drugs,p=NULL,patient,infer,type="CONTmLL",targetprior){
    Deffect=list()
    Deffect2=list()
    Deffect3=list()
    Deffect4=list()
    ...
    for (t in 1:length(celltype)) {
        ...
        if(all(drugs%in% drugname)&(dim(fcp19)[2]>2*length(drugname))) {
            ...
            Esize[t,]=Deffect3[[t]]
            dat2=fcp19[-deathid,]
            hdat2=make.R.matrix1(dat2, wt=drugs[1], pi1 = 0.01)[[1]]
            hdat=Deffect[[t]][[1]][,did,drop=FALSE]
            colnames(hdat2)=drugname[-1]
            #colnames(hdat2b)=drugname[-1]
            if(!(type %in% c("mLL", "CONTmLL"))) {
                ...
                dprop1b=p13b
                dprop1c=p13c
                #dprop3= pmax(dprop1c,pmax(dprop1,dprop1b))
                dpropa=NULL
                dprop=NULL
                dpropb=NULL
                dpropc=NULL
                for ( ro in 1: dim(dat33)[1]) dpropa=rbind(dpropa,dprop1a)
                for ( ro in 1: dim(dat33)[1]) dprop=rbind(dprop,dprop1)
                for ( ro in 1: dim(dat33)[1]) dpropb=rbind(dpropb,dprop1b)
                for ( ro in 1: dim(dat33)[1]) dpropc=rbind(dpropc,dprop1c)
                ###Deffect4[[t]]=Deffect2[[t]]+dprop
                Deffect2[[t]] <- mapply(function(x, y){logoddsf(x,y)}, as.data.frame(dat33), as.data.frame(dpropa))
                rownames(Deffect2[[t]])=rownames(dat33)
                Deffect2[[t]]=ifelse(is.na(Deffect2[[t]]),rnorm(1,10,0.05),Deffect2[[t]])
                Deffect4[[t]] <- mapply(function(x, y){logoddsf(x,y)}, as.data.frame(dat33), as.data.frame(dprop))
                rownames(Deffect4[[t]])=rownames(dat33)
                Deffect4[[t]]=ifelse(is.na(Deffect4[[t]]),rnorm(1,10,0.05),Deffect4[[t]])
                Deffect5[[t]] <- mapply(function(x, y){logoddsf(x,y)}, as.data.frame(dat33), as.data.frame(dpropb))
                rownames(Deffect5[[t]])=rownames(dat33)
                Deffect5[[t]]=ifelse(is.na(Deffect5[[t]]),rnorm(1,10,0.05),Deffect5[[t]])
                Deffect6[[t]] <- mapply(function(x, y){logoddsf(x,y)}, as.data.frame(dat33), as.data.frame(dpropc))
                rownames(Deffect6[[t]])=rownames(dat33)
                Deffect6[[t]]=ifelse(is.na(Deffect6[[t]]),rnorm(1,10,0.05),Deffect6[[t]])
                Deffect7[[t]]=pmax(Deffect6[[t]],pmax(Deffect5[[t]],Deffect4[[t]]))
                hdat3_label=Deffect7[[t]]
                for (a in 1:dim(Deffect7[[t]])[1]) {
                    for ( b in 1:dim(Deffect7[[t]])[2]) {
                        hdat3_label[a,b]=match(Deffect7[[t]][a,b],c(Deffect4[[t]][a,b],Deffect5[[t]][a,b],Deffect6[[t]][a,b]))
                    }
                }
                Deffect_label[[t]]=hdat3_label[,drugs[-1]]
                if(targetprior==TRUE) {
                ...
                dat31=round(exp(hdat2)/(1+exp(hdat2)),6)
                dat31b=round(exp(hdat2)/(1+exp(hdat2)),6)
                dat31c=round(exp(hdat2)/(1+exp(hdat2)),6)
                dat31d=round(exp(hdat2)/(1+exp(hdat2)),6)
                ...
                ###dat33=ifelse(dat32>=0.99,dat32-ss,dat32)[,drugs[-1]]
                dat33=ifelse(dat32>=0.99,dat32-ss,dat32)
                dat33b=ifelse(dat32b>=0.99,dat32b-ssb,dat32b)
                dat33c=ifelse(dat32c>=0.99,dat32c-ssc,dat32c)
                dat33d=ifelse(dat32d>=0.99,dat32d-ssd,dat32d)
                ...
                p13c=hdat
                ##p23=1/p13
                dprop1a=round(exp(p13a)/(1+exp(p13a)),6)
                dprop1=round(exp(p13)/(1+exp(p13)),6)
                dprop1b=round(exp(p13b)/(1+exp(p13b)),6)
                dprop1c=round(1/(1+exp(p13c)),6)
                dpropa=NULL
                dprop=NULL
                dpropb=NULL
                dpropc=NULL
                ...
                Deffect2[[t]]=dat33*dpropa
                Deffect4[[t]]=dat33*dprop
                Deffect5[[t]]=dat33*dpropb
                Deffect6[[t]]=dat33*dpropc
                Deffect7[[t]]=pmax(Deffect6[[t]],pmax(Deffect5[[t]],Deffect4[[t]]))
                hdat3_label=Deffect7[[t]]
                for (a in 1:dim(Deffect7[[t]])[1]) {
                    for ( b in 1:dim(Deffect7[[t]])[2]) {
                ...
                if(targetprior==TRUE) {
                    ...
                    Deffect4b[[t]]=ifelse(Deffect_label[[t]]==1,Deffect7[[t]],abs(rnorm(1,0,0.005)))
                    ...
                    #Deffect5b[[t]]=ifelse(Deffect_label[[t]]==2,Deffect2[[t]],abs(rnorm(1,0,0.05)))
                    Deffect5b[[t]]=ifelse(Deffect_label[[t]]==2,Deffect7[[t]],abs(rnorm(1,0,0.005)))
                    ...
                    #Deffect6b[[t]]=ifelse(Deffect_label[[t]]==3,Deffect2[[t]],abs(rnorm(1,0,0.05)))
                    Deffect6b[[t]]=ifelse(Deffect_label[[t]]==3,Deffect7[[t]],abs(rnorm(1,0,0.005)))
    ...
    save(Deffect4,file="Deffect4.rdata")  ### Weighted effect matrix for each subpopulation
    save(Deffect5,file="Deffect5.rdata") ### Weighted effect matrix2 for each subpopulation
    ...
    if(!(type %in% c("mLL", "CONTmLL"))) {
        ...
        if((sum(TM!=0)>1)&(length(tsort(rr$graph))>0)) {
         ...
         #  if(any(rr$mLL==Inf)) print("No Maximum likelihood profile plotted due to Inf values")
          #  else plot.nem(rr,filename=paste(patient,"Maxlikelihood for top networks for all states.pdf",sep=""),main="DRUGNEM Network",PDF=TRUE,what = "mLL",D=NULL,draw.lines = TRUE)
            ...
            plot(rr$graph,main="DRUGNEM network")
            par(mar=c(5,2,5,5))
            if (infer=="search") {
        ...
        if((sum(TMs!=0)>1)&(length(tsort(rr1$graph))>0)) {
            ...
            if(length(nemselected)!=0) {
                ...
                image(x=1:dim(pic3)[2],y=1:dim(pic2)[1],z=as.matrix(t(pic2)),col = colorpalette,xlab = "Drugs",xaxt="n",yaxt="n",ylab = "target-proteins",family="sans",cex.lab=1.4,main=paste("Heatmap","-","pooled"," drug effects", sep=""))
